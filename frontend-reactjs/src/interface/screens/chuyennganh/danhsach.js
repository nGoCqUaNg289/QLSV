...
class DanhSach extends Component {
    ...
    _getDanhSachDonVi = async (query) => {
        let data = await tbChuyenNganh.getAll(query)
    ...
    _handleDeleteMulti = async () => {
        ...
        danhsach.forEach((item, index) => {
            if (item.checked)
                axiosReq.push(tbChuyenNganh.deleteById(item._id.$oid || item._id))
        });
    ...
    _handleLockMulti = async () => {
        ...
        danhsach.forEach((item, index) => {
            if (item.checked)
                axiosReq.push(tbChuyenNganh.lockById(item._id.$oid || item._id))
        });
            ...
            return (
                <React.Fragment>
                    <div className="main portlet fade-in">
                        <BreadCrumbs title={"Danh sách chuyên ngành"} route={[{ label: 'Quản lý chuyên ngành', value: '/don-vi' }]} />
                        <div className="portlet-title">
                            <div className="caption">
                                <i className="fas fa-grip-vertical" />Danh sách chuyên ngành
                            </div>
                  ...
                  <i className="fas fa-print"></i>
                        ...
                        <div className="card">
                            <div className="card-header">
                                <Link to={'/quan-ly/chuyen-nganh/0'} className="btn btn-sm btn-outline-primary border-radius">
                                        ...
                                        <tbody>
                                            {danhsach.map((item, index) => {
                                                return <tr key={index} >
                                                    ...
                                                    <td>
                                                        <Link to={'/quan-ly/chuyen-nganh/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
