...
class DanhSach extends Component {
    ...
    _getDanhSachDonVi = async (query) => {
        let data = await tbCapBac.getAll(query)
            ...
            return (
                <React.Fragment>
                    <div className="main portlet fade-in">
                        <BreadCrumbs title={"Danh sách cấp bậc"} route={[{ label: 'Quản lý cấp bậc', value: '/ql-sinh-vien' }]} />
                        <div className="portlet-title">
                            <div className="caption">
                                <i className="fas fa-grip-vertical" />Danh sách cấp
                            </div>
                        ...
                        <div className="card">
                            <div className="card-header">
                                <Link to={'/quan-ly/ql-cap-bac/0'} className="btn btn-sm btn-outline-primary border-radius">
                                        ...
                                        <tbody>
                                            {danhsach.map((item, index) => {
                                                return <tr key={index} >
                                                    ...
                                                    <td>
                                                        <Link to={'/quan-ly/ql-cap-bac/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
