...
class DanhSach extends Component {
    ...
    _getDanhSachDonVi = async (query) => {
        let data = await tbMonHoc.getAll(query)
    ...
    _handleDeleteMulti = async () => {
        ...
        danhsach.forEach((item, index) => {
            if (item.checked)
                axiosReq.push(tbMonHoc.deleteById(item._id.$oid || item._id))
        });
    ...
    _handleLockMulti = async () => {
        ...
        danhsach.forEach((item, index) => {
            if (item.checked)
                axiosReq.push(tbMonHoc.lockById(item._id.$oid || item._id))
        });
            ...
            return (
                <React.Fragment>
                    <div className="main portlet fade-in">
                        <BreadCrumbs title={"Danh sách môn học"} route={[{ label: 'Quản lý môn học', value: '/mon-hoc' }]} />
                        <div className="portlet-title">
                            <div className="caption">
                                <i className="fas fa-grip-vertical" />Danh sách môn học
                            </div>
                        ...
                        <div className="card">
                            <div className="card-header">
                                <Link to={'/quan-ly/mon-hoc/0'} className="btn btn-sm btn-outline-primary border-radius">
                                        ...
                                        <tbody>
                                            {danhsach.map((item, index) => {
                                                return <tr key={index} >
                                                    ...
                                                    <td>{item.Ten}</td>
                                                    <td>{item.ChuyenNganh.Ten}</td>
                                                    <td>{item.KichHoat ? 'Kích hoạt' : ' '}</td>
                                                    <td>
                                                        <Link
                                                            to={{
                                                                pathname:
                                                                    "/quan-ly/mon-hoc/danhsachsv/" + item._id.$oid || item._id,
                                                        ...
                                                        <Link to={'/quan-ly/mon-hoc/' + item._id.$oid || item._id} title="Chi tiết" className="btn btn-sm btn-outline-info border-radius"><i className="fas fa-pencil-alt" /></Link>
